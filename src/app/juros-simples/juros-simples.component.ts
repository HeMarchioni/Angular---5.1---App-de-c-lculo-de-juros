...
export class JurosSimplesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formulario = this.formBuilder.group({   //-> fromGroup (construtor FormBuilder
      inicio: [null ,/* -> Valor pre existente */[Validators.required,Validators.min(0.01)]],    // -> pode colocar a validação aqui
  ...
  calculoJurosS() {
    for (let i = 1;i <= this.periodo;i++ ) {
      // @ts-ignore
      this.valoresList.push((this.valorInicio*(1 + this.taxa * i)).toFixed(2));
